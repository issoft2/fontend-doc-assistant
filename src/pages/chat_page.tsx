...
const ChatPage: React.FC = () => {
 ...
 const openConversation = useCallback(async (convId: string) => {
  ...
  const userRoleFix = (user as any)?.role || '';
...
}> = ({ isSpeaking, voices, selectedVoiceName, onVoiceChange, onStopSpeaking, onLogout, userRole  }) => {
 ...
 return (
  ...
  <header className="p-6 border-b border-slate-800/50 bg-slate-900/50 backdrop-blur-sm shrink-0">
    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
      ...
      {/* ✅ NEW: Admin Panel Link - Hidden for 'employee' */}
      {showAdminLink && (
            <a
              href="/admin"
              className="px-4 py-2 text-sm font-medium text-indigo-400 hover:text-indigo-300 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 rounded-xl transition-all duration-200 flex items-center gap-2 group"
              title="Admin Panel"
            >
